...
Checked 2026-09-08. **81 rule, stability, mechanical-motion and trackball tests passed; 29 general browser checks plus focused mechanical-art, tread, pointer-capture and inertia checks passed in each of Chrome and WebKit; no uncaught runtime errors.** Launch with `npm start`, then open [localhost:5178](http://localhost:5178). The preview is left running.
...
- **Browsers:** actual Google Chrome **152.0.7977.76** and Playwright WebKit **26.0** passed. Installed Safari **26.6.2** could not create a WebDriver session because “Allow remote automation” is disabled. No settings were changed. WebKit coverage is useful evidence, not an actual Safari pass.
